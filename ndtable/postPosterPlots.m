function postPosterPlots(allData, pNames, colateBy)
...
function plotAndFit(m, color)
    ...
    errorbar(x,yl,yl-ybl, ytl-yl, '-', 'Color', color, 'LineWidth', 2);
    hold on;
...
pt1 = get(AxesHandle,{'Position','tightinset','PlotBoxAspectRatio'});
    ...
    for i = 1:numel(ratio)
        ...
        s = 0;
        e = 2*pi*(1-ratio(i));
        circleColorDetached = [1,0,0];
        plot_arc(centerX,centerY,h,w,s,e, circleColorDetached)
        e = 2*pi;
        s = 2*pi*(1-ratio(i));
        circleColorAttached = [112, 173, 71] / 255;
        plot_arc(centerX,centerY,h,w,s,e, circleColorAttached)
    end
