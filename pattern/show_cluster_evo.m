function show_cluster_evo(series)

%show_tracking_frame(series, 8)
show_tracking(series)
%show_growth_vs_area(series)


end

function show_growth_vs_area(series)
...
for i = 1:numel(series)
    show_growth_vs_area_frame(series, i, colors(i,:));
...
function show_growth_vs_area_frame(series, idx, color)

growth = nan * zeros(numel(series(idx).clusters.areas),1);
...
for j = 1:numel(series(idx).clusters.areas)
    if (any(strcmp('prevIds',fieldnames(series(idx).clusters))) && ...
        ...
        for ii = 1:numel(series(idx).clusters.prevIds{j})
            ...
            oldArea = oldArea + series(idx-1).clusters.areas(id);
        end
        growth(j) = series(idx).clusters.areas(j) / oldArea; 
    end
    
end

areas = (series(idx).clusters.areas);
scatter(areas,growth,'.','MarkerEdgeColor',color);


vidx = find(~isnan(growth));

linearCoefficients = polyfit(areas(vidx), growth(vidx), 1);

xFit = linspace(min(areas), max(areas));
