function allData = prepareAllDataGrowth(allData)
...
for i = 1:numel(allData)
    ...
    elseif strcmpi('C5', allData(i).fileName(9:10))
        allData(i).conc = 2.2;
        allData(i).signal = 1;
    elseif strcmpi('A1', allData(i).fileName(9:10))
        allData(i).conc = 2;
        allData(i).signal = 1;
    end
end  

imageRect = [3000,5000,3000,5000];
imageRect = [];
...
allData = scoreAllData(allData, 'Seg', @getOptimalSegmentationFromBitmap, ...
    'imageName', 'I', 'laplacianSize', 4, 'clusterDilateRadius', 4, 'thresholds', [0.20:0.05:0.8],...
